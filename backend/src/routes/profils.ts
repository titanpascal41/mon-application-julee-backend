...
router.get("/", (_req, res) => {
  db.query("SELECT * FROM profils ORDER BY id ASC", (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ message: "Erreur lors de la récupération des profils", error: err.message });
    }
...
router.post("/", (req, res) => {
  ...
  db.query("INSERT INTO profils (nom) VALUES (?)", [nom.trim()], (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ message: "Erreur lors de la création du profil", error: err.message });
    }
...
router.delete("/:id", (req, res) => {
  ...
  db.query("DELETE FROM profils WHERE id = ?", [id], (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ message: "Erreur lors de la suppression du profil", error: err.message });
    }
    const info = result as ResultSetHeader;
    if (info.affectedRows === 0) {
      return res.status(404).json({ message: "Profil non trouvé" });
